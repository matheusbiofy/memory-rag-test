...
def main():
    ...
    docs = []
    for fname in sorted(os.listdir("docs")):  # Changed from "docs" to "docs_2"
        if not (fname.endswith(".txt") or fname.endswith(".md")):
            continue
        path = os.path.join("docs", fname)  # This line is already correct
    ...
    if not docs:
        print("⚠️ Nenhum documento encontrado em 'docs/'.")  # Updated folder name
        return
